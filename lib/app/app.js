...
can.Control('OpenSocial', {
  ...
  ".login-btn click":function(elm,ev){
  	Cohuman.Models.User.login({}).done(function(){
  		debugger;
  	})
  },

  task:function(token, token_secret){
  	// Bypass OAuth
  	var oauth = new Cohuman.Models.OAuth(Cohuman.config.api);
  	oauth.token = token;//'DFJU9HPbCszHbS7PQjNGo34oxfe1o6ZVK0AAdf0l';
  	oauth.token_secret = token_secret;//'BBxvrcVe30GJyXtp16CXtOzJpt8MhRmarwtj2Kiq';
  	oauth.prefilter();
  	...
  	$.when.apply($, [ Cohuman.Models.Task.findOne({ id: this.options.id || "29389" }),
		...
		this.element.append('app/views/task.ejs', {
			task: task,
			user: user,
			project: projects,
  ...
  ".add-btn click":function(elm,ev){
  	...
  	// hide followers
  	elm.closest('ul').find('.follower').toggle();

  	// trigger focus on the follower input
  	if(prev.is(':visible')){
  		prev.children('input').focus();
  	}
  },
